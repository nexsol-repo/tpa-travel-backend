package com.nexsol.tpa.core.api.external.portone;

import com.nexsol.tpa.core.api.config.PortOneV1Properties;
...
public class PortOneClient {
    ...
    public String getAccessToken() {
        ...
        synchronized (this) {
            ...
            String body = webClient().post()
                    .uri("/users/getToken")
                    .bodyValue(Map.of("imp_key", props.getApiKey(), "imp_secret", props.getApiSecret()))
                    .retrieve()
            ...
            try {
                ...
                if (code != 0) {
                    throw new RuntimeException("PortOne getToken failed. code=" + code + ", message=" + root.path("message").asText());
                }
                JsonNode res = root.path("response");
                String accessToken = res.path("access_token").asText();
                long now = Instant.now().getEpochSecond();
                long expiredAt = res.path("expired_at").asLong(now + 60); // seconds epoch

                // 만료 30초 전까지 유효로 보자(버퍼)
    ...
    public CertificationResponse getCertification(String impUid) {
        ...
        String body = webClient().get()
                ...
                .header(HttpHeaders.AUTHORIZATION, token) // PortOne V1은 Authorization: {access_token} 패턴을 사용 :contentReference[oaicite:3]{index=3}
                .retrieve()
                .bodyToMono(String.class)
                .block();
        ...
        try {
            ...
            int code = root.path("code").asInt(-1);
            if (code != 0) {
                throw new RuntimeException("PortOne getCertification failed. code=" + code + ", message=" + root.path("message").asText());
            }

            JsonNode res = root.path("response");
            CertificationResponse r = new CertificationResponse();

            // 자주 쓰는 필드들 (응답 상세 필드는 PG/상품에 따라 다를 수 있어 null-safe로)
            ...
            r.setBirthday(res.path("birthday").asText(null)); // "YYYYMMDD"
            r.setGender(res.path("gender").asText(null));     // "male"/"female" 또는 "M"/"F" 등 케이스 다양
    ...
    public static class CertificationResponse {
        ...
        public void setImpUid(String v) { this.impUid = v; }
        public void setMerchantUid(String v) { this.merchantUid = v; }
        public void setUniqueKey(String v) { this.uniqueKey = v; }
