...
public class PortOneClient {
            ...
            try {
                ...
                if (code != 0) {
                    throw new RuntimeException(
                            "PortOne getToken failed. code=" + code + ", message=" + root.path("message").asText());
                }
    ...
    public CertificationResponse getCertification(String impUid) {
        ...
        String body = webClient().get()
            ...
            .header(HttpHeaders.AUTHORIZATION, token) // PortOne V1은 Authorization:
                                                      // {access_token} 패턴을 사용
        ...
        try {
            ...
            if (code != 0) {
                throw new RuntimeException(
                        "PortOne getCertification failed. code=" + code + ", message=" + root.path("message").asText());
            }
            ...
            r.setGender(res.path("gender").asText(null)); // "male"/"female" 또는 "M"/"F" 등
                                                          // 케이스 다양
