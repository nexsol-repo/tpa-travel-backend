...
public class TravelContractQueryService {
    ...
    public Page<ContractListItem> list(String authUniqueKey, int page, int size) {
        ...
        Page<TravelContractEntity> contracts =
                (authUniqueKey != null && !authUniqueKey.isBlank())
        ...
        List<Long> planIds = contracts.getContent().stream()
                .map(TravelContractEntity::getPlanId)
                .filter(java.util.Objects::nonNull)
                .distinct()
                .toList();

        var payMap = paymentRepository.findByContractIdIn(contractIds).stream()
                .collect(java.util.stream.Collectors.toMap(TravelInsurePaymentEntity::getContractId, p -> p));

        var planMap = planRepository.findByIdIn(planIds).stream()
                .collect(java.util.stream.Collectors.toMap(TravelInsurancePlanEntity::getId, p -> p));

        var peopleMap = peopleRepository.findByContractIds(contractIds).stream()
    ...
    public ContractDetail get(Long id) {
        ...
        TravelInsurePaymentEntity pay =
                paymentRepository.findByContractId(c.getId()).orElse(null);

        TravelInsurancePlanEntity plan =
                planRepository.findById(c.getPlanId()).orElse(null);

        TravelInsurerEntity insurer =
                insurerRepository.findById(c.getInsurerId()).orElse(null);

        TpaPartnerEntity partner =
                partnerRepository.findById(c.getPartnerId()).orElse(null);

        TpaChannelEntity channel =
                channelRepository.findById(c.getChannelId()).orElse(null);
